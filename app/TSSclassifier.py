...
def classify(gff_df, tss_list, confidence_list, strand):
    """
    Classifies TSS based on their position relative to genes in a GFF file. :param gff_df:  DataFrame containing
    information from a GFF file :param tss_list: a list of all predicted TSS :param confidence_list: list that
    assigns a confidence value to each of the TSS in tss_list :param strand: the strand on which the TSS were
    predicted(True for reverse and False for forward) :return: tss_classified: data frame that contains the position,
    classified type, confidence value, distance to the next directed gene and corresponding gene name of the TSS
    """
    tss_classified = []
    ...
    classified_df = pd.DataFrame(tss_classified, columns=['Pos', 'TSS type', 'confidence', 'gene name', 'distance to '
                                                                                                        'next '
                                                                                                        'directed '
                                                                                                        'gene'])
...
def find_common_tss(prediction, master_table, strand):
    ...
    :return: common_tss_df: data frame that contains the TSSs that were found by both TSS predator and our prediction
    """
    if not strand:
